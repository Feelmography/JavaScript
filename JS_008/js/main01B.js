...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const scoreCheck = () => {
    ...
    for (let i = 0; i < inputScores.length; i++) {
       ...
       * 1번 조건문이 실행되지 않았으면
       * 2번 조건문 검사를 하고,
       *   2번 조건문이 실행 = 점수가 0보다 작고 100보다 큰 경우
       *   "점수 범위..." alert 이 뜰 것이고
       *   실행은 input.focus() 로 점프한다
       * 1번, 2번 조건문이 모두 통과 = 점수가 정상이다
       *   else 에 있는 continue 가 실행되고
       *   다음 점수를 검사를 시도한다
       * 만약 1번, 2번 검사에서 한 번이라도 문제를 찾으면
      ...
      // 입력된 box 의 문자열을 clear 하여
      // 다시 입력을 쉽게 할 수 있도록 도와주는 역할
      // input.value = "";

      // 입력된 text 에 선택된 block을 설정하여
  ...
  const scorePrint = () => {
    ...
    // table 의 tbody에 이미 추가된 모든 tr, td 요소 제거
